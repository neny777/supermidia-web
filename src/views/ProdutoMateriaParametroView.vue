...
const onSubmit = async (values) => {
    try {
        ...
        const item = { ...materiasCalculo[itemIndex] };
        const parametros = [...(item.parametros || [])];

        if (validarDuplicidade(parametros, values.codigo, parametroIndexAtual)) {
            showToast('erro', `O parâmetro ${values.codigo} já foi informado para esta matéria do produto.`);
            return;
        }
        ...
        materiasCalculo[itemIndex] = item;
                            ...
                            <div class="position-relative">
                                ...
                                <Form v-if="state.formReady" @submit="onSubmit" :validation-schema="schema" :initial-values="state.parametro" v-slot="{ values }">
                                    ...
                                    <div class="card-footer text-center">
                                        ...
                                        <button type="button" class="btn btn-primary button-medium m-2" @click="router.push({ name: 'produto-materia-editar', params: { produtoId: route.params.produtoId, itemIndex: route.params.itemIndex } })">
