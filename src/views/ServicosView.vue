...
const fetchServicos = async () => {
    try {
        ...
        showToast("erro", "Erro ao carregar serviços.");
    } finally {
...
const deleteServico = async (id) => {
    const modal = showModal("Excluir serviço", "Confirma a exclusão do serviço?", async () => {
        try {
            state.isProcessing = true;
            await axiosInstance.delete(`/servicos/${id}`);
            state.servicos = state.servicos.filter((servico) => servico.id !== id);
            redrawTable('list-servicos');
            showToast("sucesso", "Serviço excluído com sucesso.");
        } catch (error) {
                                    ...
                                    <div class="col-md-12">
                                        <div class="table-responsive p-2">
                                            <div v-if="state.isProcessing" class="text-center">
                                                <DataTables id="list-servicos" :options="options"
                                            ...
                                            <div v-else>
                                                <DataTables id="list-servicos" :options="options"
                                                    ...
                                                    <tbody>
                                                        <tr v-for="servico in state.servicos" :key="servico.id">
                                                            <td class="d-none">{{ servico.id }}</td>
                                                            <td>{{ servico.nome }}</td>
                                                            <td>{{ servico.unidade }}</td>
                                                            <td>R$ {{ Number(servico.preco).toFixed(2) }}</td>
                                                            <td class="text-center">
                                                                <button class="btn btn-primary btn-sm mx-2"
                                                                    @click="router.push({ name: 'servico', params: { servicoId: servico.id } })">
                                                                    <i class="bi bi-pen"></i>
                                                                </button>
                                                                <button class="btn btn-danger btn-sm mx-2"
                                                                    @click="deleteServico(servico.id)">
