...
export const codigosParametro = ['FATOR', 'ACRESCIMO_ALTURA', 'ACRESCIMO_LARGURA', 'ESPACAMENTO', 'QUANTIDADE_FIXA'];
...
const normalizeVinculos = (vinculos) =>
    (vinculos || []).map((vinculo) => ({
        medidaNome: vinculo.medidaNome ?? '',
        multiplicador:
            vinculo.multiplicador === '' || vinculo.multiplicador == null ? 1 : Number(vinculo.multiplicador),
    }));

export const normalizeParametros = (parametros) =>
...
const normalizeMedidas = (medidas) =>
    (medidas || []).map((medida) => ({
        nome: medida.nome ?? '',
...
export const buildItemSchema = (tipoItem, calculos) =>
    yup.object({
        ...(tipoItem === 'materia'
            ? {
                  materiaId: yup
        ...
        parametros: yup
            ...
            .test('parametros-obrigatorios', 'Parâmetros obrigatórios não informados.', function (parametros = []) {
            ...
            .test('parametros-valores', 'Valores inválidos.', function (parametros = []) {
